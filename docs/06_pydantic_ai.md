...
Pydantic AI 是由 Pydantic 团队开发的新一代 AI Agent 框架，于 2024 年发布，2026 年已发展到 v2.0。它以 **类型安全** 和 **开发者体验** 为核心设计理念，将 Pydantic 强大的数据验证能力与 LLM 调用深度整合，让开发者用最少的代码构建类型安全的 AI Agent。

与 LangChain/LangGraph 不同，Pydantic AI **以 Agent 为中心** — 一切功能（工具调用、结构化输出、依赖注入、流式输出）都围绕 Agent 这个核心概念展开。它不是 LangChain 的替代品，而是针对不同场景的补充工具。

### 学习目标

完成本章学习后，你将能够：

1. 理解 Pydantic AI 的核心设计理念和与 LangChain 的定位差异
2. 使用 `Agent` 类快速构建带工具调用的 AI Agent
3. 利用 Pydantic 模型实现类型安全的结构化输出
4. 通过 `deps_type` 和 `RunContext` 实现依赖注入
5. 定义同步和异步工具函数，构建工具链
6. 使用 `run_stream()` 实现流式输出
7. 构建多 Agent 协作系统（委托模式、路由模式）
8. 在面试中清晰回答 Pydantic AI vs LangChain 的对比问题
...
### Q1: Pydantic AI 和 LangChain 的 Agent 有什么区别？

**标准答案：**

这是面试中最高频的对比问题。核心区别在于**设计哲学**：

| 维度 | Pydantic AI | LangChain |
|------|------------|-----------|
| **中心概念** | Agent 是一等公民 | Chain/Graph 是核心 |
| **工具调用** | 内置循环，自动处理 | 需要 AgentExecutor |
| **结构化输出** | output_type 自动验证 | 需要手动 OutputParser |
| **依赖注入** | deps_type + RunContext | 无内置方案 |
| **代码量** | 约 1/3（简洁） | 约 3x（模板代码多） |
| **生态** | 轻量聚焦 | 丰富（向量DB、文档加载器） |
| **类型安全** | 编译时 + 运行时 | 仅运行时 |

**深入解释：**

1. **设计哲学不同**：LangChain 以 Chain/Graph 为中心，Agent 只是工作流中的一种运行模式（AgentExecutor + ReAct 循环）。Pydantic AI 以 Agent 为中心，一切功能（工具调用、验证、重试）都围绕 Agent 展开。

2. **类型安全**：Pydantic AI 利用 Pydantic v2 的强大类型系统，输出类型验证在编译时和运行时都可用。LangChain 需要手动引入 PydanticOutputParser，且格式指令需要在 Prompt 中显示传递。

3. **代码量**：实现同样的结构化输出功能，Pydantic AI 通常只需 5-7 行核心代码，而 LangChain 需要定义 Parser、Prompt、Chain，约 15-20 行。

### Q2: Pydantic AI 的依赖注入系统如何工作？

**标准答案：**

Pydantic AI 的依赖注入系统是受 FastAPI 启发设计的，它通过三步完成：

1. **声明依赖类型**：通过 `Agent(deps_type=MyDeps)` 声明依赖的类型（用于静态类型检查）

2. **运行时传递实例**：通过 `agent.run_sync('...', deps=my_deps_instance)` 传递依赖实例

3. **工具中访问依赖**：在 `@agent.tool` 装饰的函数中，通过 `ctx: RunContext[MyDeps]` 访问 `ctx.deps`
...
@agent.tool
def query(ctx: RunContext[DatabaseDeps], sql: str) -> str:
    return ctx.deps.connection.execute(sql)
```

**对比 FastAPI**：Pydantic AI 的依赖注入与 FastAPI 的 `Depends()` 机制非常相似，都是通过类型注解声明依赖，运行时注入。区别在于 Pydantic AI 使用 dataclass 一次性传递所有依赖，而 FastAPI 使用函数式依赖链。

### Q3: 如何在 Pydantic AI 中实现结构化输出？

**标准答案：**

Pydantic AI 的结构化输出是其最大的特色功能，通过以下方式实现：
...
result = agent.run_sync('分析这段文字的情感')
analysis: AnalysisResult = result.output  # 类型安全！
```

**核心特性：**

- **自动 Schema 生成**：Pydantic 模型自动转换为 JSON Schema，发送给 LLM
- **自动验证**：LLM 返回的 JSON 自动验证并转换为 Pydantic 对象
- **自动重试**：验证失败时自动重新请求 LLM（通过 `retries` 参数控制）
- **支持复杂类型**：嵌套模型、Enum、Optional、List、Union 等
- **自定义验证器**：通过 `@field_validator` 添加业务逻辑验证

### Q4: Pydantic AI 的流式输出机制是怎样的？

**标准答案：**

Pydantic AI 提供三层流式输出，从粗到细：

1. **stream_text()** — 文本增量流：适合聊天界面
```python
async with agent.run_stream('讲故事') as stream:
    async for text_chunk in stream.stream_text():
        print(text_chunk, end='')
```

2. **stream_output()** — 结构化输出增量流：输出类型的部分更新
```python
async with agent.run_stream('...') as stream:
    async for partial_output in stream.stream_output():
        # partial_output 是 output_type 的增量更新
```

3. **stream_events()** — 完整事件流：包含工具调用、模型请求等所有事件
```python
async with agent.run_stream('...') as stream:
    async for event in stream.stream_events():
        # 包含 PartStartEvent, ToolCallPart, FinalResultEvent 等
```

**内部机制**：Pydantic AI 的流式输出基于异步生成器（AsyncGenerator）。当 LLM 开始流式返回时，Agent 逐步解析 JSON 增量，同时监听工具调用事件，自动处理工具调用循环并持续流式输出结果。
...
```python
# ❌ 错误：声明了 deps_type 但运行时未传递
agent = Agent('openai:gpt-4o', deps_type=MyDeps)
result = agent.run_sync('...')  # 运行时错误！

# ✅ 正确：运行时传递 deps
result = agent.run_sync('...', deps=MyDeps(...))
```
...
result = agent.run_sync('...')  # 可能正常工作但不推荐
...
```python
# ❌ 避免：过于宽泛的输出类型
class TooGeneric(BaseModel):
    data: dict  # LLM 难以生成正确的任意 dict
...
```python
# ❌ 默认 retries=1，复杂工具可能不够
agent = Agent('openai:gpt-4o')

# ✅ 根据工具复杂度设置合理的重试次数
agent = Agent('openai:gpt-4o', retries=3)
# 或分别设置工具和输出重试
agent = Agent('openai:gpt-4o', retries={'tools': 3, 'output': 2})
```

### 最佳实践总结

1. **使用 Pydantic 模型定义输出类型**，不要用 `dict` 或 `Any`
2. **充分利用依赖注入**传递数据库连接、API 客户端等
3. **为工具函数添加完整的类型注解和 docstring**（LLM 依赖这些信息选择工具）
4. **合理设置 retries**：简单任务 1-2 次，复杂任务 3-5 次
5. **使用 'test' 模型进行单元测试**，无需真实 API
6. **结合 LangGraph 做复杂编排**：Pydantic AI Agent 作为 LangGraph 节点

## 与 LangChain/LangGraph 集成方案
...
```python
# LangGraph 节点中使用 Pydantic AI Agent
from pydantic_ai import Agent
...
analyzer = Agent(
    ...
    system_prompt='分析用户查询并提取结构化信息。',
)

def analyze_node(state: State) -> dict:
    result = analyzer.run_sync(state['query'])
    return {'structured_result': result.output.model_dump()}
...
1. Pydantic AI 的 Agent 包含哪些核心组件？
2. `@agent.tool` 和 `@agent.tool_plain` 的区别是什么？
3. 如何在 Pydantic AI 中实现依赖注入？
4. Pydantic AI 的结构化输出验证失败时会发生什么？
5. 什么场景下应该选择 Pydantic AI 而不是 LangGraph？
6. 如何实现 Pydantic AI 与 LangGraph 的组合使用？
7. Pydantic AI 的流式输出有哪三层？分别适用于什么场景？

::: details 参考答案
1. Agent = 模型(Model) + 工具(Tools) + 输出类型(output_type) + 系统提示(system_prompt) + 依赖(deps_type)
2. `@agent.tool` 需要 `RunContext` 参数访问依赖；`@agent.tool_plain` 是纯函数工具，无需上下文
3. 通过 `deps_type` 声明依赖类型，`run_sync(deps=...)` 传递实例，`RunContext[Type].deps` 在工具中访问
4. 自动重新请求 LLM（重试次数由 `retries` 参数控制）
5. 简单 Agent 任务、需要强类型安全、快速原型开发、结构化数据提取
6. LangGraph 做流程编排（StateGraph），Pydantic AI Agent 作为节点执行
7. `stream_text()` 逐词输出（聊天界面）、`stream_output()` 结构化增量（类型安全的实时更新）、`stream_events()` 完整事件流（调试和可观测性）
:::
